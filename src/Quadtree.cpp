...
void Quadtree::AddObject(Vertex *object) {
	if (level == maxLevel) {
...
vector<Vertex*> Quadtree::GetObjectsAt(float _x, float _y) {
	if (level == maxLevel) {
		return objects;
	}
	
	vector<Vertex*> returnObjects, childReturnObjects;
	if (!objects.empty()) {
...
bool Quadtree::contains(Quadtree *child, Vertex *object) {
